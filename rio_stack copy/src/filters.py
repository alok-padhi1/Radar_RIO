...
def compute_doppler_static_probability(xyz: np.ndarray, v_radial: np.ndarray,
                                       v_body_hint: np.ndarray | None,
                                       omega_hint: np.ndarray | None,
                                       cfg: FilterConfig,
                                       lever_arm: np.ndarray | None = None) -> np.ndarray:
    """Computes the probability that points are static based on Doppler consistency.

    Blueprint §11.2, §27: Replace hard point deletion with quality weighting.
    Instead of a binary keep-mask, returns a static probability in [0, 1].
    """
    r = np.linalg.norm(xyz, axis=1)
    r = np.clip(r, 1e-3, None)
    u = xyz / r[:, None]
    if v_body_hint is None or np.linalg.norm(v_body_hint) < 0.1:
        return np.ones(len(xyz), dtype=float)
    ...
    # Soft probability weighting: 1.0 if residual == 0, decays to 0.0
    # using a Gaussian-like or linear falloff based on doppler_eps_mps
    prob = np.clip(1.0 - (residual / cfg.doppler_eps_mps), 0.0, 1.0)
    return prob
...
class PreprocessResult:
    ...
    n_raw: int
    n_after_range: int
    n_after_doppler: int  # Maintained for API compatibility, but is now soft
    n_after_persistence: int
    n_after_sor: int
    n_final: int
    static_probabilities: np.ndarray = None  # Added: per-point static probabilities
...
def preprocess_frame(xyz_body: np.ndarray, v_radial: np.ndarray,
    ...
    # 2. Soft Doppler Gate (Blueprint §11.2, §27)
    # We no longer hard-delete points based on Doppler. We compute a probability.
    static_probs = compute_doppler_static_probability(xyz1, v1, v_body_hint, omega_hint, cfg, lever_arm=lever_arm)
    xyz2 = xyz1
    ...
    xyz3 = xyz2[keep]
    static_probs_3 = static_probs[keep]
    n_after_persistence = len(xyz3)

    # 4. Statistical Outlier Removal
    pcd = o3d.geometry.PointCloud()
    pcd.points = o3d.utility.Vector3dVector(xyz3)
    pcd, idx = statistical_outlier_removal(pcd, cfg)
    n_after_sor = len(pcd.points)
    
    if len(idx) > 0:
        static_probs_4 = static_probs_3[idx]
    else:
        static_probs_4 = np.empty(0)

    # 5. Voxel Downsample
    # To maintain static probabilities through voxel downsampling without writing a custom
    # voxelizer, we can interpolate them back or just accept the loss of probability granularity
    # for the visualization path. For the flight path, the submap manager handles raw points.
    ...
    return PreprocessResult(
        ...
        static_probabilities=static_probs_4  # Pre-voxel downsample probabilities
    )
