...
# LIO-SAM Lessons Incorporated — v2 Architecture Addendum

This addendum explicitly incorporates the useful architectural ideas from the original TixiaoShan/LIO-SAM repository while keeping FAST-LIO2 as the real-time Livox Avia LIO front-end.

## 1. What we take from LIO-SAM

### A. Separate real-time estimation from slower graph optimization

LIO-SAM maintains two factor-graph processes: one long-lived graph for mapping/GPS-related optimization and another periodically reset IMU/lidar graph intended to preserve real-time odometry. We should adopt the architectural principle, not copy the ROS1 implementation.

Our implementation:

```text
FAST-LIO2
  └── protected real-time LIO estimator
       ├── high-rate pose/velocity
       ├── local ikd-Tree registration
       └── navigation output
...
Background backend
  ...
  ├── GPS consistency factors
  └── persistent map stitching
```

The backend must never block the FAST-LIO2 estimator.

LIO-SAM's repository describes its mapping factor graph as continuously maintained while its IMU-preintegration factor graph is reset periodically for real-time operation. This is the idea to retain. 
...
Use LIO-SAM's concept of not saving every LiDAR frame as a global optimization node.

Create a keyframe when at least one of these is exceeded:
...
The exact thresholds must be tuned from flight logs.
...
pose covariance
velocity
IMU bias estimate if available
downsampled local point cloud
descriptor
submap membership
...
This is derived from the LIO-SAM pattern of maintaining key poses, keyframe clouds and pose transforms rather than requiring the entire raw cloud history for every optimization step.

## 3. Local submaps

Do not perform loop closure against the entire map.
...
place recognition
        ↓
retrieve candidate historical keyframes
        ↓
fuse 10–50 nearby historical keyframes
        ↓
coarse registration
        ↓
fine registration
...
LIO-SAM explicitly searches historical keyframes and fuses a limited number of nearby historical frames into a submap for loop closure. Its parameters include a historical search radius, minimum time separation, number of history frames and ICP fitness threshold.

For the Avia system, these become configurable parameters rather than hard-coded values.
...
loop_closure:
  enabled: true
  frequency_hz: 0.5
  min_keyframe_separation_m: 15.0
  min_time_separation_s: 30.0

  candidate_search_radius_m: 30.0
  candidate_count: 10

  local_submap_keyframes: 20
  coarse_voxel_m: 0.5
  fine_voxel_m: 0.15

  max_registration_rmse_m: 0.35
  min_inlier_ratio: 0.30
```

Do not copy the above numeric thresholds into flight certification. They are starting values for controlled experiments.

## 4. Loop closure: use LIO-SAM's graph mechanism, improve its detector

LIO-SAM demonstrates the important backend sequence:
...
We should keep this exact conceptual structure.

However, its original loop closure is explicitly described as a proof-of-concept ICP method. The repository recommends Scan Context for more advanced loop closure.

Therefore our implementation is:

```text
FAST-LIO2 keyframe
       ↓
Scan Context / geometric descriptor
       ↓
candidate retrieval
       ↓
distance + temporal gating
       ↓
...
Never add a loop factor merely because descriptor similarity is high.

The loop must pass geometric verification first.

## 5. GPS should be a factor/measurement, not a hard pose switch

LIO-SAM already contains a useful design pattern: GPS is added to the graph only when its covariance and temporal conditions are acceptable.

For our system, make this stronger:
...
```text
              ...
              GPS integrity gate
             /        |         \
        healthy    uncertain    bad
           │           │          │
           ▼           ▼          X
        factor       monitor    reject
           │
           ▼
     global graph
```

GPS quality checks should include:
...
velocity jump
time continuity
agreement with FAST-LIO2
agreement with flight-controller estimate
```

For spoofing suspicion:

```text
GPS position - SLAM predicted position
```

must be evaluated against uncertainty.

If the innovation is too large for the stated covariance, do not immediately snap the vehicle to GPS.

When GPS returns after an outage:
...
compare GPS vs SLAM
    ↓
if consistent:
    add/re-enable global GPS constraint
else:
    classify GPS as suspect
```

This follows the LIO-SAM philosophy of validating GPS covariance before graph insertion, but extends it for GPS-denied/spoof-aware operation.

## 6. Timestamp discipline: adopt LIO-SAM's strict requirement

This is one of the most important items to import.

LIO-SAM's preprocessing explicitly requires point timestamps for deskewing and states that time synchronization problems can cause severe trajectory problems.
...
as a replacement for missing sensor measurement timestamps.
...
Expose synchronization diagnostics in the flight log.

## 7. IMU extrinsics and frame conventions

LIO-SAM's documentation places strong emphasis on the distinction between:

```text
raw IMU measurement rotation
IMU attitude rotation
LiDAR-to-IMU extrinsic rotation
```

We should preserve this discipline.

Create one authoritative calibration file:
...
Do not maintain separate hand-written transformations in different nodes.

All nodes must use the same TF/extrinsic source.

## 8. IMU validation procedure

Before any flight, build the validation utility suggested by the spirit of LIO-SAM's IMU debugging guidance.
...
6. rotate -pitch
7. rotate +yaw
8. compare expected vs reported signs
9. verify LiDAR-frame transformation
```
...
ATTITUDE_FRAME_OK
EXTRINSIC_ROT_OK
EXTRINSIC_TRANSLATION_OK
TIMESTAMP_OK
```

A failed check blocks autonomous navigation mode.

## 9. Map storage: improve on simple PCD saving

LIO-SAM's save-map service is useful as a reference for reconstructing a global map from corrected keyframe poses.

We should improve this for flight:
...
**the authoritative map is the keyframe/submap database + optimized poses; the stitched PCD is a generated product.**

Do not make one enormous PCD file the primary localization database.

## 10. Existing-map localization / relocalization

Add an explicit mode that loads the previously generated map:
...
load map metadata
 ↓
load nearby descriptors
 ↓
FAST-LIO2 initializes local odometry
 ↓
place recognition against saved database
 ↓
candidate map submap
 ↓
LiDAR registration
 ↓
recover global map pose
 ↓
initialize map↔odom transform
 ↓
continue local FAST-LIO2
```

This lets the aircraft use a previously mapped area rather than starting a completely new arbitrary local map every time.

## 11. Corrected-map regeneration

When GTSAM moves historical poses after loop closure:

```text
optimized poses
       ↓
reproject stored keyframe/submap clouds
       ↓
voxel merge
       ↓
update spatial tiles
       ↓
write corrected tile versions
```

Never simply move the current visualization cloud and call that the corrected persistent map.

LIO-SAM's `correctPoses()` logic explicitly updates historical key poses after graph optimization and clears/rebuilds cached map structures. We should retain that concept.

## 12. Two map resolutions

Your current Avia guide emphasizes dense mapping configuration. Keep that for the offline/background map, but separate it from the navigation representation.
...
This is especially important on the Jetson Orin Nano Super.

## 13. Exact runtime partition
...
PX4 odometry interface
watchdog
```

Must remain operational if the background stack crashes.
...
These processes must be restartable.

## 14. Important difference from the uploaded radar slam_node.py

Do not reuse the radar-specific mechanisms from the existing `slam_node.py`.

That file contains:

```text
U300 radar point format
radar Doppler
RIO velocity
radar tilt transform
radar-specific GICP
AGL correction
radar UDP packets
```

The Livox architecture does not need those mechanisms.

The reusable ideas are only general software patterns:

```text
health/status reporting
bounded queues
latest-value sensor updates
explicit reject reasons
pose confidence metrics
safe shutdown
persistent logging
```

## 15. Antigravity implementation rule

Antigravity must NOT:

```text
clone LIO-SAM and replace random files
```

Instead it must:

```text
1. start from the working FAST-LIO2 Avia setup
2. preserve FAST-LIO2 estimator
3. add a ROS2 keyframe manager
4. add a submap database
5. add Scan Context candidate retrieval
6. add registration verification
7. add GTSAM/iSAM2 backend
8. add GPS integrity manager
9. add map database
10. add PX4 state-source manager
11. add relocalization
12. add watchdogs
```

Use LIO-SAM as an architectural reference for:
...
loop factors
iSAM2 update
historical pose correction
global-map regeneration
map saving
IMU/timestamp discipline
```

## 16. Recommended package structure

```text
~/ros2_ws/src/gps_denied_lio/
├── avia_interface/
├── fast_lio_adapter/
├── lio_keyframe_manager/
├── lio_submap_server/
├── scan_context/
├── loop_verifier/
├── graph_backend/
...
/fastlio/odometry
/fastlio/odometry_cov
/fastlio/local_map

/lio/keyframe
/lio/keyframe_pose
/lio/submap
/lio/localization_status
/lio/health
...
## 18. Navigation-source state machine
...
GPS_PRIMARY
    |
    | GPS invalid / timeout / inconsistency
    v
LIO_PRIMARY
    |
    | LIO unhealthy
...
Do not switch based on a single packet.

## 19. Flight-control output contract

The navigation bridge must publish:
...
pose covariance
twist covariance
source = GPS / LIO / fused
validity flags
```

PX4 should receive a clearly defined external odometry message, with covariance populated from the estimator rather than arbitrary fixed values.

## 20. Jetson memory policy

Do not allocate arbitrary unbounded Python lists for point clouds.
...
keyframe cache:
    bounded, e.g. 256–512 MB

loop candidate cache:
    bounded, e.g. 128–256 MB

background map cache:
    bounded, e.g. 512 MB–1.5 GB

NVMe:
    authoritative persistent store
```

Actual RAM budgets must be profiled on the exact image, ROS2 build and Livox stream before flight.
...
CPU load
GPU memory
FAST-LIO2 loop latency
background queue depth
NVMe write latency
dropped frames
```

No process is permitted to consume memory indefinitely.

## 21. CPU scheduling rule

The navigation chain gets priority.

Background tasks must yield whenever:

```text
FAST-LIO2 latency exceeds threshold
ROS queue depth rises
CPU load exceeds threshold
memory pressure rises
NVMe write stalls
```

Map writing must therefore be asynchronous.

## 22. Validation additions from LIO-SAM's own guidance

Create mandatory automated tests for:

```text
[ ] Livox timestamps present
[ ] IMU timestamps present
[ ] LiDAR/IMU time offset measured
[ ] LiDAR→IMU extrinsic verified
[ ] attitude frame verified
[ ] gravity direction verified
[ ] ROS frame convention verified
[ ] no NaN point cloud
...
[ ] GPS outage replay
[ ] GPS recovery replay
[ ] GPS inconsistency replay
[ ] map save/reload
[ ] map relocalization
[ ] Jetson memory stress
[ ] background backend crash recovery
```

## 23. Required regression datasets
...
Dataset B:
hand-carried straight line

Dataset C:
hand-carried loop

Dataset D:
full mapped-area replay
...
Dataset G:
GPS jump/inconsistency injection

Dataset H:
long-duration Jetson memory stress
```

The most important acceptance criterion is repeatability:

```text
same input bag
    →
same trajectory within specified tolerance
    →
same loop closures
    →
same map topology
...
## 24. Final implementation choice
...
FAST-LIO2
    +
LIO-SAM-inspired keyframe architecture
    +
Scan Context
    +
GTSAM/iSAM2
...
GPS integrity layer
    +
persistent NVMe submap/tile database
    +
PX4 external odometry interface
```

Do NOT use:

```text
LIO-SAM ROS1 as the primary estimator
```

Do NOT use:

```text
the old radar slam_node.py as the Livox estimator
```

Do NOT use:

```text
one giant in-RAM PCD as the authoritative map
```

Do NOT use:

```text
one-packet GPS switching
```

Do NOT let:

```text
loop closure / map saving / map loading
```

block the real-time FAST-LIO2 navigation path.

### Sources

- TixiaoShan/LIO-SAM: https://github.com/TixiaoShan/LIO-SAM
- FAST-LIO2: https://github.com/hku-mars/FAST_LIO
- Scan Context reference linked by LIO-SAM: https://github.com/irapkaist/SC-LeGO-LOAM
